...
async def load_and_parse_release(
    ...
    if len(releases) > 1:
        diff_response = await client.get(
            ...
            f"{releases[1].get('tag_name', '')}...{releases[0].get('tag_name', '')}.diff",
        ...
        commits_raw = await client.get(
            f"https://api.github.com/repos/{repo_name}/commits"
        )
        commits = json.loads(commits_raw.text)

        diff_response = await client.get(
            f"https://github.com/{repo_name}/compare/"
            f"{commits[-1].get('sha', '')}...{releases[0].get('tag_name', '')}.diff",
            headers=diff_headers,
        )
        return GitHubRelease(
            repo=repo_name,
            description=diff_response.text,
